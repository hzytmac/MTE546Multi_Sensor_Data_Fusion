...
%% import everything
import1=load('exp3.mat');
voltagemeasure=transpose(import1.data);
import2=load('time.mat');
time=transpose(import2.time);

% Discrete time step
dt = 0.1;

% Prior (initial assumption)
xhat = [9.8969;3]; % mean (mu)
...
plot(T,y_hat(1,:));
plot(time,voltagemeasure(1,:));
title('Short Range IR Sensor Measurement')
legend('Measurement', 'Prediction','Actual');

subplot(2,2,4)
hold on
plot(T,y(2,:)) %Measurement
plot(T,y_hat(2,:));
plot(time,voltagemeasure(2,:));
